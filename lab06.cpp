...
void Pociag::operator()() {
    for (int i = 0; i < 3; ++i) {
        ...
        if (peron.czyPusty())   //pociąg próbuje wjecahć na peron preferwany
        {
            peron.przyjazd(*this);  //peron wolny więc wjeżdża
            this_thread::sleep_for(chrono::milliseconds(zakres(generator)));    //pasażerowie wysiadają
            peron.odjazd(*this);    //pociąg odjeżdża
        } else //jeśli peron zajęty to wjeżdża na zapasowy
        {
